                 ...
                 eMBRegisterMode eMode )
{
  ...
  //偏移量
  int16_t         iRegIndex;
  uint16_t        dataReadVlaue,dataWriteVlaue;
  
  //判断寄存器是不是在范围内
  if( ( (int16_t)usAddress >= REG_HOLDING_START ) \
        && ( usAddress + usNRegs <= REG_HOLDING_START + REG_HOLDING_NREGS ) )
  {
      if (usNRegs > gUsAddrMax)
      {
          eStatus = MB_EINVAL;
          return eStatus;
      }
    ...
    switch ( eMode )
    {
      ...
      case MB_REG_READ:
        while( usNRegs > 0 )
        {
          dataReadVlaue = HoldRegList_mb_Read(iRegIndex);
          *pucRegBuffer++ = ( uint8_t )( dataReadVlaue >> 8);
          *pucRegBuffer++ = ( uint8_t )( dataReadVlaue & 0xFF );
          iRegIndex++;
          usNRegs--;
        }
        break;
      ...
      case MB_REG_WRITE:
        while( usNRegs > 0 )
        {
          dataWriteVlaue = (uint16_t)*pucRegBuffer++ << 8;
          dataWriteVlaue |= (uint16_t)*pucRegBuffer++;
          eStatus = HoldRegList_mb_Write(iRegIndex,dataWriteVlaue);
          iRegIndex++;
          usNRegs--;
        }
        break;
